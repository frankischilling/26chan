...
from resource_fixture import ENVIRONMENT, OwnedFixture, private_json, wait_for
...
def anchor(root, binary, tools):
    ...
    child = subprocess.Popen([sys.executable, str(Path(__file__).with_name('resource_qualify.py')),
...
def check_cleaned(root, state, qualifier_pid, supervisor):
    ...
    for unit in state['units']:
        if unit not in (fixture.fixture_unit, fixture.observer_unit):
        ...
        if state_after.get('ActiveState') in ('active', 'activating', 'deactivating'):
            raise AssertionError('Owned resource service survived cleanup')
    ...
    if fixture.mount_info() or fixture.mount.exists() or fixture.private.exists() or fixture.public.exists():
        raise AssertionError('Owned mount or temporary files survived cleanup')
    for port in state['ports']:
        with socket.socket() as probe:
            probe.settimeout(1)
            if probe.connect_ex(('127.0.0.1', port)) == 0:
                raise AssertionError('Owned resource qualification listener survived cleanup')
...
def main(binary, tools, interrupt=False):
    ...
    if not binary.is_absolute() or not binary.is_file() or not tools.is_absolute():
        raise AssertionError('Supply absolute existing resource qualification binaries')
    ...
    root = Path('/var/lib') / ('board-resource-monitor-' + secrets.token_hex(8))
    ...
    try:
        ...
        with os.fdopen(descriptor, 'wb') as log:
            ...
            def healthy():
                if result_path.exists() or supervisor.poll() is not None:
                    raise AssertionError('Native resource qualifier exited before healthy authenticated scrape')
            ...
            if len(state['children']) != 2 or len(set(state['children'])) != 2:
                raise AssertionError('Native resource qualification did not own two monitoring children')
            for pid in [supervisor.pid, qualifier_pid, *state['children']]:
                if os.getpgid(pid) != supervisor.pid or os.getsid(pid) != supervisor.pid:
                    raise AssertionError('Native resource child escaped its owned group')
            ...
            if work.parent != fixture.private or not (work / 'pki').is_dir() or not (fixture.private / 'observer.env').is_file():
                raise AssertionError('Owned private resource credentials and PKI were not exercised')
            if state['mount'] != str(fixture.mount) or not fixture.mount_info():
                raise AssertionError('Owned resource tmpfs was not exercised')
            if interrupt:
            ...
            if json.loads(result_path.read_text()) != (143 if interrupt else 0):
                raise AssertionError('Resource qualification did not complete its expected cleanup path')
            ...
            if supervisor.wait(timeout=5) != 0:
                raise AssertionError('Owned resource supervisor did not acknowledge successful cleanup')
            print((root / 'qualification.log').read_text(), end='')
            print('PASS ' + ('OS SIGTERM' if interrupt else 'normal stop')
                  + ' removed owned services, child processes, listeners, tmpfs and temporary credentials', flush=True)
        ...
        # This root watcher also owns the exact units/mount if an inner process
...
if __name__ == '__main__':
    ...
    if args.anchor is not None:
        ...
        try:
            ...
            print('Owned resource qualification or cleanup failed; private inputs are not logged.', file=sys.stderr)
