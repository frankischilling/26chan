...
def find_notification(alerts, labels, status):
    for alert in alerts:
        ...
        if not alert.get('fingerprint'):
            raise AssertionError('Resource notification has no fingerprint')
        return alert
    return None


def assert_same_alert(firing, resolved):
    if (firing['fingerprint'] != resolved['fingerprint'] or firing['labels'] != resolved['labels']
            or firing.get('startsAt') != resolved.get('startsAt')
            or firing['status'] != 'firing' or resolved['status'] != 'resolved'):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='stack-', dir=fixture.private) as directory, ExitStack() as cleanup:
            ...
            for url, basic in ((prom_url + '/api/v1/status/runtimeinfo', prom_basic),
                               ('https://localhost:' + str(alert_port) + '/api/v2/status',
                                ('operator', credentials['alert-operator']))):
            ...
            def notification(name, status, dimension, target):
                while True:
                    try:
                ...
                return find_notification(seen, labels, status)
            ...
            def alert_cycle(name, enable, disable, native_high, native_low, dimension, target):
                ...
                auth.wait_for('healthy control before ' + name,
                              lambda: not query('ALERTS{job="board-resource",alertname="' + name + '"}'), children, 55)
                notification(name, 'resolved', dimension, target)  # Drain prior stages.
                seen[:] = [alert for alert in seen if alert.get('labels', {}).get('alertname') != name]
                enable()
                try:
                    auth.wait_for('native ' + name + ' pressure', lambda: native_high(scrape()), children, 25)
                    firing = auth.wait_for(name + ' firing HTTPS notification',
                                           lambda: notification(name, 'firing', dimension, target), children, 55)
                    ...
                    disable()
                auth.wait_for('native ' + name + ' recovery', lambda: native_low(scrape()), children, 25)
                resolved = auth.wait_for(name + ' resolved HTTPS notification',
                                        lambda: notification(name, 'resolved', dimension, target), children, 65)
                assert_same_alert(firing, resolved)
                auth.wait_for('nonfiring ' + name, lambda: not query('ALERTS{job="board-resource",alertname="' + name
                              + '",alertstate="firing"}'), children, 15)
            ...
            try:
                ...
                auth.wait_for('native unavailable source and readiness503', unavailable, children, 20)
                firing = auth.wait_for('unavailable resource HTTPS notification',
                                      lambda: notification('BoardResourceObserverUnavailable', 'firing', None, None), children, 40)
            finally:
                fixture.mount.chmod(0o711)
            auth.wait_for('restored actual source', lambda: metric(scrape(), 'board_resource_sample_success') == 1, children, 20)
            resolved = auth.wait_for('unavailable resource resolved HTTPS notification',
                                     lambda: notification('BoardResourceObserverUnavailable', 'resolved', None, None), children, 40)
            assert_same_alert(firing, resolved)
