...
async function fixture(page, stored = {}, replies = { [a]: 30, [b]: 10, [c]: 20, [d]: 0 }) {
  ...
  const card = (id, index) => {
    ...
    return `<section class="thread" id="thread-${id}" data-thread-id="${id}" data-bumped="${100 + index}" data-latest-reply="${index === 0 ? '9007199254741099' : id}" data-replies="${replies[id]}" data-sticky="${id === d}"><a class="catalogThumb" href="/test/thread/${id}" data-search-text="<b>${subject}</b>: ${subject}" data-search-file="" data-has-file="false"><img class="thumb nofile" id="thumb-${id}" src="/static/catalog/nofile.png" width="77" height="13" data-small-width="77" data-small-height="13" data-large-width="77" data-large-height="13"></a><div class="meta" id="meta-${id}" title="(R)eplies / (I)mage Replies">R: <b>${replies[id]}</b></div><div class="teaser">${subject}</div></section>`;
  };
