// Prepared catalog fields enter a fresh worker; user patterns never run in the client path.
export const FILTER_LIMITS = Object.freeze({
  filters: 64, patterns: 1024, boardText: 1024, boardToken: 32, boards: 32,
  posts: 512, field: 16384, settings: 131072, request: 2097152, response: 65536,
  deadline: 1000,
});

const types = new Set([0, 1, 2, 4, 5, 6]);
const fields = ['trip', 'name', 'comment', 'id', 'sub', 'filename'];
...
function checkedJob(value) {
  ...
  const posts = value.posts.map(post => {
    ...
    for (const field of fields) {
      ...
      if (typeof text !== 'string' || text.length > FILTER_LIMITS.field) throw new Error('invalid-field');
      length += text.length;
