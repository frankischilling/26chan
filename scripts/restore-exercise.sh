...
GRANT CONNECT ON DATABASE :"restore_db" TO board_public, board_migrator;
SQL
...
for table in content.boards content.threads content.reports post_secrets.deletion staff_identity.accounts staff_identity.credentials deployment.settings public._sqlx_migrations; do
...
SQL
printf 'Restore exercise passed: post fingerprint, eight table counts, public read, staff denial. Disposable restored database removed.\n'
