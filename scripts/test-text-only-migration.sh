...
upgrade_db="imageboard_text_only_$(date +%s)_$RANDOM"
[[ $upgrade_db =~ ^imageboard_text_only_[0-9]+_[0-9]+$ ]] || exit 1
created=0
cleanup() {
  if [[ $created = 1 ]]; then
...
for migration in migrations/*.sql; do
  [[ $migration != migrations/0033_text_only_policy.sql ]] || break
  "${db[@]}" --single-transaction -f "$migration"
done
"${db[@]}" <<'SQL'
-- All 82 active source board filenames. Only news enables TEXT_ONLY.
...
SQL
"${db[@]}" --single-transaction -f migrations/0033_text_only_policy.sql
"${db[@]}" <<'SQL'
DO $$ BEGIN
  IF (SELECT count(*) FROM content.boards) <> 82
    OR EXISTS(SELECT 1 FROM content.boards WHERE text_only IS DISTINCT FROM (slug = 'news'))
  THEN RAISE EXCEPTION 'Text-only source defaults differ'; END IF;
  IF EXISTS(SELECT to_jsonb(b)-'text_only' FROM content.boards b EXCEPT SELECT to_jsonb(b) FROM public.owned_boards_before b)
...
DO $$ BEGIN
  IF (SELECT text_only FROM content.boards WHERE slug='future') IS DISTINCT FROM false
  THEN RAISE EXCEPTION 'Future boards did not inherit the global default'; END IF;
  BEGIN
    UPDATE content.boards SET text_only=NULL WHERE slug='future';
    RAISE EXCEPTION 'Text-only policy became nullable' USING ERRCODE='ZX001';
  EXCEPTION WHEN not_null_violation THEN NULL; END;
END $$;
UPDATE content.boards SET text_only=true WHERE slug='future';
DO $$ BEGIN
  IF NOT (SELECT text_only FROM content.boards WHERE slug='future')
  THEN RAISE EXCEPTION 'Operator could not enable subject policy'; END IF;
END $$;
UPDATE content.boards SET text_only=false WHERE slug='future';
SQL
...
DO $$ BEGIN
  IF (SELECT text_only FROM content.boards WHERE slug='future') IS DISTINCT FROM false
    OR NOT (SELECT text_only FROM content.boards WHERE slug='news')
  ...
  BEGIN
    UPDATE content.boards SET text_only=false WHERE slug='news';
    RAISE EXCEPTION 'Public changed subject policy' USING ERRCODE='ZX001';
  EXCEPTION WHEN insufficient_privilege THEN NULL; END;
  BEGIN
    ALTER TABLE content.boards DROP COLUMN text_only;
    RAISE EXCEPTION 'Public changed subject schema' USING ERRCODE='ZX001';
  EXCEPTION WHEN insufficient_privilege THEN NULL; END;
END $$;
SQL
cleanup
echo 'Text-only upgrade passed: 82 source defaults, retained historical content/clocks, operator changes and denied public policy/schema writes. Disposable database removed.'
