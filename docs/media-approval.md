...
Interrupted publication leaves private, unapproved bytes. Cleanup uses the same storage lock, selects at most 64 abandoned/deleting reservations, rechecks each lease, removes only that generated object's fixed filenames, syncs the directory, and then removes the deleting record. Retrying cleanup is safe after interruption at either boundary. Active leases remain ineligible. A preceding bounded retirement pass can retire deleted/expired attachments or unused approvals older than one day through a restricted function; live attachments and usable receipts are preserved. The coordinator cannot directly alter an approved record or revive a deleting record. Each attempt uses its reserved ID for its full/thumbnail staging files, so recovery needs no directory scan. See the [retention contract](post-attachments.md#output-retention-and-cleanup).
...
Create `board_media_read` with `sudo bash scripts/dev-media-reader-db.sh` before applying migration 0008. Reuse existing `.local` credentials if already provisioned. The migration preserves legacy job receipts and deliberately creates no approvals for old files. Those files need explicit validation and reconciliation; a receipt alone never grants access. Existing installations need the bootstrap-only `deploy/media-retention-role.sql` before migration 0016. The development retention policy does not establish production scheduling, backup erasure or recovery guarantees.
