...
| 7.3.2, 7.4.1, 7.4.2, 7.4.4 | Eight-hour absolute lifetime, logout, live revocation check, visible logout control | Inactivity timeout and deployment-specific lifetime policy |
...
This mapping identifies relevant implementation work; it does not mark any full ASVS chapter satisfied. In particular, hardware attestation, recovery identity proofing, inactivity timeout, deployed media containment and operations remain unverified or incomplete. [Verification](verification-media-staff.md) records which checks actually ran.
