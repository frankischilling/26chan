...
| 2.4.1 | Public peer limiter, staff authentication-start limiter, request admission held through response data lifetime, body limits, bounded previews and media queue admission; shared public/API connection admission with header and absolute connection deadlines; per-response encoded-byte ceilings and a shared retained-output pool | Database/result/view/intermediate memory, transport and consumer copies, distributed abuse control, and deployed host/proxy resource, timeout and load qualification |
