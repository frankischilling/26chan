...
Later checkpoints add [durable publication fencing](media-approval.md) and [live network/storage witnesses](verification-media-boundaries.md). The witness suite creates synthetic metadata, DNS, internal/proxy/external-address services, a management socket and private files, verifies allowed access before and after fourteen guest denials, and rejects incomplete probe reports.

Authenticated bounded queue dispatch is implemented and exercised with the actual decoder in the owned native harness. Remaining requirements include a maintained dedicated processing tier; deployed dispatch identities and certificate operations; per-attempt identity allocation for concurrent jobs; deployed restart/power-loss qualification; denial checks against actual owned deployment destinations; full resource saturation qualification; media-origin serving; complete format compatibility; and independent security review. Local synthetic witnesses do not qualify deployed routing or a production processing tier.
