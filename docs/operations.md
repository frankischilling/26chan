...
Terminate public HTTPS at the [verified Linux Unix-socket proxy](public-proxy.md). Production requires the explicit socket path and dedicated proxy worker UID; the application checks kernel credentials before accepting the client-address header. Configure public, staff and media origins explicitly. If the optional [API listener](api.md) is enabled, give it a separate HTTPS server block forwarding only to API_BIND_ADDR. Keep staff ingress separate with its own host-only session cookies; media requires a separate registrable domain. The candidate proxy config and owned HTTPS test cover client identity, while firewall policy, certificates, service accounts and actual production deployment still require operator qualification.
...
| Dynamic encoded output | 32 MiB per response; 128 MiB of shared payload blocks in 4096-byte units | Pre-allocation reservations and real router/database regressions cover exhaustion, frame ownership and recovery; database snapshots and intermediate models need separate memory qualification |
...
Comment limits count Rust `chars()` and PostgreSQL `char_length` in a UTF8 database. New comments normalize CRLF and bare CR to LF before the board character check; combining marks count separately. [Source spacing cleanup](source-comment-spacing.md) then applies locked operator policy while retaining independent storage ceilings. Existing text is unchanged. A maximum comment can occupy 64,000 bytes before HTML escaping. The [encoded response pool](public-request-limits.md#encoded-response-storage) reserves final payload blocks before allocation and retains them through delivery. Database snapshots, parsed comments and intermediate view/JSON models remain outside it. Mixed load and those allocations still need qualification against the candidate OS memory ceiling.

Admission remains occupied after a handler returns while its response body or emitted data is retained. The final body layer runs after API error/HEAD/OPTIONS transformations; data clones and slices share the permit owner. Dropping an unconsumed response or releasing its completed data frees capacity. Empty responses release immediately. This request-count lease and the encoded-block reservations have separate lifetimes, and neither acknowledges that the client received data. Small overload responses, kernel buffers and copies made by downstream consumers are outside them. See [admission verification](verification-response-admission.md) and the output pool's scope above.
