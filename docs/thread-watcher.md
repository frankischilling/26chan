...
### Network-level cookie rejection

`post-tracking-cookie-policy.spec.js` uses isolated, disposable full-Chromium profiles with `profile.default_content_setting_values.cookies` set to Allow (1) and Block (2). It runs as a separate server batch in `test:behavior`, without changing request limits. Both profiles submit real opted-in OP and board-return reply forms. The allowed profile is the healthy receipt/watch/tracking control. The blocked profile must receive the actual Set-Cookie response headers and DevTools `Network.responseReceivedExtraInfo.blockedCookies` entries with the `UserPreferences` reason for each receipt. No request interception, header stripping, document.cookie replacement or application policy bypass supplies this result.

Posts remain readable through the real JSON API while rejected cookies never enter the cookie jar. The blocked profile is then closed, its owned cookie preference is restored, and the same profile is reopened. Previously rejected receipts must not appear as deferred own-post tracking or automatic watches. The fixture deletes only its created threads through their deletion passwords and removes only its generated profile directories after the browser closes. Blocking all cookies may also make web storage unavailable; settings are therefore enabled through their existing same-tab fallback before each blocked submission.

The full Chromium executable (`channel: chromium`) is intentional: an exploratory probe found that the default headless-shell executable accepted cookies despite the profile setting. The full-browser probe on Chromium 151.0.7922.34 stored the healthy cookie and rejected the blocked cookie with `UserPreferences`. Browser installation remains pinned through the existing Playwright package and CI installation. This covers first-party cookie rejection in that Chromium policy, not all browser privacy settings or third-party-cookie behavior. The separate disabled-document.cookie test continues to cover a different case where receipts can remain in the browser jar for later consumption.

Primary tool references: [Playwright persistent profiles](https://playwright.dev/docs/api/class-browsertype#browser-type-launch-persistent-context) and [DevTools blocked-cookie reasons](https://chromedevtools.github.io/devtools-protocol/tot/Network/#type-SetCookieBlockedReason). No personal browser profile is opened or modified.
...
`tests/media-visual/post-menu.spec.js` covers the production-template fixture's desktop/mobile menus, provider URLs, keyboard behavior, deletion-form selection, absent file controls and hostile/noncanonical URLs. The JavaScript branches of `public-upload.mjs` exercise the same controls after actual approved publication and perform password-gated file-only deletion through the menu-selected form, followed by real media-reader revocation checks. Third-party search results and availability are not qualified by these owned tests. Native thread hiding, recursive reply hiding and remaining menu/settings/reference-capture work are still unfinished.

### Drag cancellation and queued-save qualification

`watcher-position-races.spec.js` adds eight read-only browser cases in a separate `test:behavior` server batch. A DevTools-generated touch cancel and an explicit release of actual pointer capture must restore the starting position without saving; tests require trusted browser events and first demonstrate a successful drag. Changing fixed mode in another tab must also cancel an active drag.

The queued-save cases hold the actual watcher Web Lock, confirm that a position save is pending, then release it and acquire the same lock again as a completion barrier. A healthy control must save the new position while preserving unrelated settings written by another tab. Newer coordinates, a fixed-mode change and global disabling must prevent an older queued position from being committed. Navigation to another real board view must abandon a waiting save without changing persisted coordinates. These checks supplement the existing malformed-coordinate, basic cross-tab, mobile restoration and optional-storage cases; they do not assert crash-atomic storage or complete native visual parity.

The cancellation event uses the pinned Playwright protocol binding for [Input.dispatchTouchEvent](https://chromedevtools.github.io/devtools-protocol/tot/Input/#method-dispatchTouchEvent), including an empty touch-point list for touchCancel. No application request limit or runtime code is changed by this qualification slice.
