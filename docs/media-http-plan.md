...
- [x] Add subprocess configuration cases in `crates/config/tests/media_http.rs`
...
- [x] Add `MediaHttpSettings::from_env()` in `crates/config/src/media_http.rs`,
  reexport it, and add `MediaAdminSettings::group_read`. Preserve current CLI
  defaults and configuration denials.
- [x] Add Unix storage tests for private mode, rejected unprovisioned shared
...
- [x] Add workspace crate `apps/media-http`, tests and empty router interface;
  record a failed real approval/HTTP expectation before runtime implementation.
- [x] Implement `AppState::new(reader, files, origin)` and `router(state)` with GET/HEAD
...
- [x] Test actual pending/approved/corrupt/missing/removed approval states,
...
- [x] Add the executable using typed configuration and graceful shutdown. Build
...
- [x] Add `deploy/media-http.service` and an owned qualification test under
...
- [x] Wire the native test into CI after dispatch qualification. Update README,
...
- [x] Run formatting, warnings-denied workspace Clippy, locked builds/tests,
  public/staff/browser checks, advisory checks and actual native qualification.
- [x] Obtain source review and fix findings.
- [ ] Push a focused PR referencing the issue,
