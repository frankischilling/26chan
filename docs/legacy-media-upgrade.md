...
Both directories must already exist, must not be symlinks or overlap, and must belong to the reviewed storage layout. `CLIENT_CONFIG` is a private authenticated dispatcher configuration provisioned for the offline operator. On Unix, the dispatcher accepts configuration/credential files only from root or the command's effective UID; a root operator cannot use nonroot coordinator-owned files directly. Provision operator-owned material through the reviewed certificate procedure, without changing runtime ownership or supplying migration credentials to the coordinator. `ASSET_ID` is one exact 32-character lowercase hexadecimal identifier selected by the operator. There is no directory scan, automatic batch selection or raw-file fallback. The command rejects production mode; a production deployment needs separate qualification and approval.
...
The local TLS server supplies fixed synthetic RGBA output and is not decoder-containment evidence. `scripts/test-media-dispatch.sh --http` adds a native upgrade case using the real gateway, Firecracker decoder, canonical store and separately identified HTTP reader. It checks exact original preservation, equivalent manifests, reader storage restrictions, retry and guest cleanup. The first [native run on `3878165`](https://github.com/frankischilling/26chan/actions/runs/34735269238) rejected the offline root command's coordinator-owned dispatcher files, as required by the existing Unix ownership check. The fixture tests that refusal before using separate root-owned copies of its synthetic certificate/configuration; it verifies that coordinator credential bytes, ownership and modes remain unchanged. The correction and complete native dispatch/intake/restore checks passed [PR CI on `9a8e5b1`](https://github.com/frankischilling/26chan/actions/runs/34735844313). Production filesystem/secret/network policy, independent review and full original-site compatibility remain outside this owned-runner result.
