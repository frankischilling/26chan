...
| B-002 | `sage`, current-count bumping and source cutoff; **source**: [original rules](#counts-bumping-and-admission) | Count/sticky/permaage/permasage and age bump rules implemented, serialized per board | [Source bump rules](source-bump-rules.md), [audited flag controls](thread-bump-flags.md) and [request-start age cutoff](source-bump-age.md). Surviving replies include the incoming post; sticky/permasage override permaage, which overrides age. Flag CI passed; new age persistence/migration checks await current-head CI. Lifetime reply admission and self-bump policy remain unfinished |
...
equality suppressing the ordinary bump. OP self-bumps
...
| B-002 | Sage substring processing, current-count cutoff, sticky suppression and [persisted permaage/permasage controls](thread-bump-flags.md) passed CI. [Request-start age suppression](source-bump-age.md) is implemented; new persistence/migration qualification awaits current-head CI. Self-bump and admission rules remain unfinished. |
