...
| B-002 | `sage`, current-count bumping and source cutoff; **source**: [original rules](#counts-bumping-and-admission) | Count/sticky/permaage/permasage bump rules implemented, serialized per board | [Source bump rules](source-bump-rules.md) and [audited flag controls](thread-bump-flags.md). Surviving replies include the incoming post; sticky/permasage override permaage. Persisted additions await current-head CI. Lifetime reply admission and age/self-bump policy remain unfinished |
...
| B-002 | Sage substring processing, current-count cutoff, sticky suppression and [persisted permaage/permasage controls](thread-bump-flags.md) are implemented; flag qualification awaits current-head CI. Age/self-bump and admission rules remain unfinished. |
