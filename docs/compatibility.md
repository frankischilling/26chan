...
The [source text-only policy](source-text-only.md) implements the supplied
`news` default, ordinary post-time subject/reply-file rules, native form/body
attributes and integer board JSON flags. Migration 0033 preserves historical
content. This extends B-001 and I-001; actual database/browser/migration and
restore qualification is pending current-head CI. Media-disabled development
advertisement remains distinct from source board policy.
...
| E-004 | Original active-thread unique_ips is a count of visible host values including the OP, conditionally exposed; archives clear those values; [source detail](#read-only-api) | Unique counts omitted. [Private OP matching](source-op-bumps.md) retains only the active OP address and same-address reply membership, removed on deletion/archive | Minimizes retained addresses while implementing self-bump rules. No invented unique count or public identity field; clients expecting unique_ips remain unsupported. Compromised public credentials can read active OP records; staff/media credentials cannot. |
...
[Posting timestamps](source-posting-times.md) persist whole-second server
request time for posts and public thread modification metadata. Bump ordering
retains the database clock. HTTP validators have a separate change clock so
an older request committing last does not hide newer content. Historical
creation times are retained and public runtime UPDATE grants exclude them.
...
through separate implemented branches. [Age suppression](source-bump-age.md)
uses request-start seconds and the OP post timestamp, with zero disabled and
equality suppressing the ordinary bump. [OP self-bumps](source-op-bumps.md)
...
The [Rust catalog projection](source-catalog-teasers.md) applies those break,
spoiler, SJIS and `/b/` branches to the saved formatter's typed output. Display
and GET/live search share that prepared representation. Source link spelling,
its effect on the short `/b/` length threshold, ambient mbstring encoding,
filename serialization and complete text-only layout remain open in #82.
...
| V-005 | Source break/spoiler/SJIS/b-truncation projection is implemented on the saved formatter. Source links, ambient encoding and full text-only layout remain open. |
...
| V-009 | Display and search share source teaser preparation; remaining link/filename spelling and full text-only matching stay in #82. |
