...
| B-002 | `sage`, current-count bumping and source cutoff; **source**: [original rules](#counts-bumping-and-admission) | Count/sticky/permaage/permasage, age and OP self-bump rules implemented, serialized per board | [Count rules](source-bump-rules.md), [flag controls](thread-bump-flags.md), [age cutoff](source-bump-age.md) and [private OP matching](source-op-bumps.md). Sticky/permasage override permaage, which overrides age and self-sage. Flag CI passed; new age and OP persistence/migration/restore checks await their current-head CI. Source admission, other cooldowns and full timestamp assignment remain unfinished |
...
| B-002 | Sage substring processing, current-count cutoff, sticky suppression and [persisted flags](thread-bump-flags.md) passed CI. [Age suppression](source-bump-age.md) and [private OP matching](source-op-bumps.md) are implemented; new persistence/migration/restore qualification awaits their current-head CI. Other cooldowns, complete timestamp assignment and admission rules remain unfinished. |
