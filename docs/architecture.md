...
| Queue observer / `board_monitor` | SELECT on one owner-backed aggregate view; no job, content, staff, deployment or storage access | Can read fixed aggregate counts/ages, consume its one database connection or falsify its own metrics if compromised. It cannot read individual jobs or change queue state through its login. Private scrapes use an independent Bearer token; production network/OS enforcement and downstream monitoring authentication remain unqualified. See [queue observation](queue-observability.md). |
