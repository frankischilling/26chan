...
The separate Playwright configuration starts staff and public applications with their own runtime credentials. The fixture executable requires migration authority and exists only under `examples/`. Browser traces, screenshots and videos are disabled to avoid recording invitation/session material. The browser test uses a Chromium virtual authenticator and harmless disposable records. It checks enrollment/login, real persisted moderation and audit entries, CSRF and stale-session denials, inactivity expiry with cookies retained, activity without recent-authentication renewal, logout, recovery, and public ETag invalidation. Synthetic authentication does not demonstrate physical authenticator protection.
