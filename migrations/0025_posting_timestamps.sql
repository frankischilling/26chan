-- Posting clocks are supplied only for new rows; no UPDATE of creation time.
GRANT INSERT(created_at) ON content.posts TO board_public,board_attachment_owner;
GRANT INSERT(created_at,modified_at) ON content.threads TO board_public;

-- Public source last_modified can regress when an older request commits last.
-- Keep HTTP validators on a separate database change clock.
ALTER TABLE content.threads ADD COLUMN http_modified_at timestamptz NOT NULL DEFAULT clock_timestamp();
CREATE FUNCTION content.advance_http_clock() RETURNS trigger
LANGUAGE plpgsql SET search_path = pg_catalog, pg_temp AS $$
BEGIN
    NEW.http_modified_at := GREATEST(OLD.http_modified_at, clock_timestamp());
    RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION content.advance_http_clock() FROM PUBLIC;
CREATE TRIGGER advance_http_clock BEFORE UPDATE ON content.threads
FOR EACH ROW EXECUTE FUNCTION content.advance_http_clock();
...
DECLARE
    ...
    v_asset record;
BEGIN
    IF p_posted_at IS NULL OR NOT isfinite(p_posted_at) THEN
        RAISE EXCEPTION 'Invalid posting timestamp.' USING ERRCODE = '22023';
    END IF;
    ...
    INSERT INTO content.posts(id, board, thread_id, name, subject, comment, created_at)
        VALUES (p_id, p_board, p_thread, p_name, p_subject, p_comment, date_trunc('second', p_posted_at));
...
END
$$;
REVOKE ALL ON FUNCTION content.insert_post_attachment(bigint,text,bigint,text,text,text,text,text,boolean,timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION content.insert_post_attachment(bigint,text,bigint,text,text,text,text,text,boolean,timestamptz) TO board_public;

-- Older binaries have no request timestamp argument. Preserve their entry
-- point and all security checks, using current database seconds as fallback.
CREATE OR REPLACE FUNCTION content.insert_post_attachment(
    ...
    SELECT content.insert_post_attachment(p_id,p_board,p_thread,p_name,p_subject,
        p_comment,p_job,p_capability,p_spoiler,clock_timestamp());
$$;
REVOKE ALL ON FUNCTION content.insert_post_attachment(bigint,text,bigint,text,text,text,text,text,boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION content.insert_post_attachment(bigint,text,bigint,text,text,text,text,text,boolean) TO board_public;
RESET ROLE;
